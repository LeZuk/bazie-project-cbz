...
**Authors:** Maksymilian Oliwa & Piotr Pazdan
...
*   Scheduling Conflict Prevention - The most important feature of the system is the prevention of time overlaps. The database enforces logic that prevents the assignment of a student or instructor to two classes happening at the same time.
*   Data Standardization - Database implements a basic data normalization (e.g. normalizing phone numbers to E.164 format). This makes working with the database easier and more flexible.
*   Enrollment-Grade Integrity - Database resolves the issue of "orphaned" grades by strictly enforcing that marks can only be assigned to students enrolled in the corresponding course.

**Database Schema**
![](misc/dbdiagram-light.png)

### 2. Database Structure

The data model is divided into four logical components: Organization, People, Scheduling and Grading.
...
*   Grading - Student performance is tracked via the `marks` table. Data quality is provided  using CHECK constraints that restrict input values to the university grading scale (2.0, 3.0, 3.5, 4.0, 4.5, 5.0).
...
Security is managed via a Role-Based Access Control model with three levels:
...
*   Hierarchical Coding - Strict formatting is enforced on identification codes to mirror the database structure. Major codes (`XXX-000`) serve as prefixes for Course codes, which in turn prefix Group codes (`XXX-000-XXX-000-XXX-000`).
